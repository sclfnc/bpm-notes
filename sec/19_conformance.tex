...
Four candidate WF-nets are checked against $L_{full}$ (same names as the process-mining gallery, different nets). $N_1$ is the $\alpha$-discovered net: after $a$, the examination ($b$ or $c$) runs in parallel with the ticket check $d$; $e$ joins them; then pay $g$, reject $h$, or reinitiate $f$, which loops back into both branches:
\begin{center}
\begin{tikzpicture}
...
Instead of stopping at the first problem, replay to the end, forcing disabled transitions to fire, and count the damage.
...
The tags read causally: $\mathbf{m}$ means ``$d$ occurred in the log, impossible in the model here''; $\mathbf{r}$ means ``$d$ was due in the model, absent from the log''.
\par\smallskip
...
The event-level score sits well above the naïve case-level one ($0.9504$ vs $0.6815$ for $N_2$): cases that fail as wholes keep most events replayable. Read it informally as ``about $95\%$ of events replay''; the tie to the true proportion is only indirect (swapping two adjacent events costs one missing plus one remaining token).
\end{definitionbox}
...
\subsection{Comparing footprints}

Optional reading: an alternative to token replay that compares the model's footprint against the log's. The \emph{play-out} technique generates from a workflow net a locally complete set of firing traces: the smallest set exercising every directly-follows pair the net can produce. Read that set as a log and its footprint is the footprint of the \emph{model}, comparable cell by cell with a log's (\emph{complete} when every activity pair that can follow another does so at least once).
