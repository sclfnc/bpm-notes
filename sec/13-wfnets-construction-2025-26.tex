...
% ── slides p.2–p.3 ──────────────────────────────────────────
The previous chapter's pipeline (build the net, derive $N^*$, check liveness and boundedness) is \emph{verification}: design first, check later. The opposite philosophy is \emph{construction}: assemble the workflow from pre-validated pieces with composition rules that preserve the desired properties, so the result is sound \emph{by construction}. Two ingredients: a catalogue of \textbf{building blocks}, small WF nets easily proved sound and \emph{safe} (1-bounded), and \textbf{composition patterns} under which sound-and-safe is closed. Together they form a calculus: every net produced by repeated application inherits the properties for free.
...
\begin{definitionbox}{Refinement $N[N'/t]$ and composition theorem}
...
\par\smallskip
\textbf{Theorem} (proof sketched below). If $N$ and $N'$ are sound and safe, so is $N[N'/t]$.
\end{definitionbox}
...
The initial transitions of $N'$ take over the consuming role of $t$ at its input place, the final transitions its producing role at the output place. \emph{Proof idea of the theorem}: a sound WF net behaves like a transition whose consumption and production are not atomic: one token in, eventually one token out, nothing else across the interface. Formally one exhibits a bijection between the reachable markings of $N[N'/t]$ and the interface-respecting pairs of reachable markings of $N$ and $N'$; safeness and soundness of the composite follow from those of the parts.
...
A second family reifies branching as transitions. Here one caution: a single transition never makes a choice: firing it marks \emph{all} its output places at once, which is an AND, not an XOR. The choice must sit on a place. So the \emph{explicit XOR-split} is an entry transition feeding one internal place from which two alternative transitions compete (the free-choice place carries the decision, exactly as in the implicit XOR); the \emph{explicit XOR-join} is the dual; and the combined \emph{XOR block} chains an explicit split, two alternative tasks and an explicit merge. The \emph{AND block} is drawn like the XOR block but its entry transition produces \emph{both} internal places simultaneously: $t$ and $t'$ then interleave freely and the exit transition consumes both. Split as concurrency, not as choice: that single difference in the entry transition's post-set separates AND from XOR. The catalogue is open: any net proved sound and safe may join it.
...
\emph{Refinement} grows nets top-down. Start from the basic block. Replace its one task by a sequence. Replace a task of that result by another sequence. Replace still others by iteration and XOR blocks. After a few such rounds the net is a structured combination of sequences, a loop and an exclusive branch: sound and safe at every intermediate step, with no global verification ever run.

\emph{Abstraction} is the dual, and the practical proof technique: to certify a given net, recognise sub-patterns matching catalogue blocks, collapse each to a single task, repeat. A worked reduction: an iteration block collapses first, then an explicit XOR block along the bottom, then a sequence in the upper branch, another iteration, another sequence; the two residual branches between the AND-split and AND-join form a parallel block, which collapses to the \emph{basic} block: the structural certificate that the original net was sound and safe.
\par\smallskip
The word \emph{abstraction} carries three distinct senses across the course, and naming them apart keeps this reduction calculus from being confused with the others. \textbf{Horizontal abstraction} moves \emph{along} the control flow at one level: collapse a whole block (a sequence, an XOR-choice, an AND-parallelism, an iteration) into a single node, the inverse of refinement, which is exactly the calculus above. \textbf{Vertical abstraction} moves \emph{across} hierarchy levels: a task hides an entire sub-net, and collapsing or expanding it changes the level of detail: the sub-process construction of the Petri-nets chapter (a transition realised by a whole workflow net) and the collapsed/expanded sub-processes of BPMN. \textbf{Aggregated abstraction} folds \emph{many elements into one} summary rather than a block or a level: many measurements collapse into one aggregate value, as with the KPI aggregation functions of the quantitative-analysis chapter, or a discovered process map is read at a coarser level of detail in process mining. Horizontal and vertical abstraction are structure-preserving and reversible: collapse then expand recovers the original; aggregation is lossy, since a single summary cannot in general be resolved back into the elements it condensed.

% ── slides p.30–p.34 ──────────────────────────────────────────
\begin{examplebox}{Design example: Car Damage}
...
The wiring replicates locally the connection pattern $t$ had towards the rest of $N$; the single-input-single-output rule is the special case of singleton ${}^\bullet t$ and $t^\bullet$. A fifth catalogue block exploits the new freedom, \emph{AND with sync}: an AND-split forks two places, a central transition consumes \emph{both} and produces \emph{both} the places consumed by the final AND-join:
\begin{center}
\begin{tikzpicture}[node distance=0.3cm and 0.55cm]
  \node[bpmn event] (i) {};
