...
% ── slides p.2–p.6 ──────────────────────────────────────────
Two net properties, written $N \vdash \psi$ (``the net $N$ satisfies $\psi$''), are studied formally: \emph{boundedness}, already met, caps the tokens on every place; \emph{liveness} guarantees that no transition becomes permanently disabled.\footnote{Desel J., Esparza J., \emph{Free Choice Petri Nets}, Cambridge University Press, 1995, \url{https://www7.in.tum.de/~esparza/bookfc.html}.} Two logical tools recur. To disprove an implication $P \Rightarrow Q$ one exhibits a witness with $P \wedge \neg Q$: small Petri nets serve as counterexamples throughout. And reachability composes: $M \in [M_0\rangle \wedge M' \in [M\rangle \Rightarrow M' \in [M_0\rangle$, so ``for any reachable marking'' always means ``reachable from $M_0$'' without ambiguity.

% ── slides p.7–p.14, p.20–p.22, p.27–p.30, p.35–p.38 ──────────────────────────────────────────
...
Intuitively, $t$ is live when at no point of any computation can we exclude that $t$ will fire again: from every reachable marking some continuation re-enables it. The $\forall\forall\exists$ alternation (over every transition, then every reachable marking, then some continuation) is what gives the property bite: it must not be confused with the much weaker $\exists M : M \xrightarrow{t}$, which only says $t$ is not dead. Live and dead are extremes, not complements: a transition can be non-live and non-dead at once: able to fire now, yet able to lose that ability forever.
...
\end{tikzpicture}
\end{center}
Left: $[M_0\rangle = \{p_1, p_2\}$ and from $p_2$ nothing is enabled. $t_1$ is non-dead (enabled at $M_0$) and non-live (after firing, disabled forever): both negative properties at once. Right: closing the cycle with $t_2 : p_2 \to p_3$ and $t_3 : p_3 \to p_1$ makes every transition live (from each of $p_1, p_2, p_3$ the marking enabling any chosen transition is reachable along the cycle) so the net is live.
\end{examplebox}

% ── slides p.23–p.26, p.31–p.34 ──────────────────────────────────────────
...
\end{tikzpicture}
\end{center}
\emph{Left net.} The token can loop on $p_1 \rightleftarrows p_2$, but once $t_3$ moves it to the $p_4 \rightleftarrows p_5$ loop there is no way back: from any marking in $\{p_4, p_5\}$ the transitions $t_1, t_2, t_3$ are forever disabled. Live: $t_4, t_5$ (the right loop is reachable from everywhere and cycles); non-live: $t_1, t_2, t_3$; dead: none (each fires at least once from $M_0$). The net is \emph{not} live. Placewise, identically: $p_4, p_5$ are live, $p_1, p_2$ non-live, none dead: not place-live.
\par\smallskip
\emph{Right net.} The back transition $t_6 : p_5 \to p_1$ closes the loop: every transition and every place can be re-engaged from every reachable marking. The net is live and place-live.
\end{examplebox}
...
For a marking $M$ not necessarily initial: $t$ is \textbf{dead at $M$} when $\forall M' \in [M\rangle : M' \not\xrightarrow{t}$, and $p$ is \textbf{dead at $M$} when $\forall M' \in [M\rangle : M'(p) = 0$; taking $M = M_0$ recovers the global notions. Three facts: a node dead at some reachable $M$ is non-live; conversely, non-liveness \emph{means} the existence of such an $M$ ($\mathsf{NonLive} \Leftrightarrow \exists M\, \mathsf{Dead}(\cdot, M)$); and deadness is \emph{monotone}, once dead at $M$, dead at every marking in $[M\rangle$: the set of dead nodes can only grow during a run. Throughout, nets have no \emph{isolated} nodes (${}^\bullet n = n^\bullet = \emptyset$), which would be forever disabled or unmarked by construction.
\end{definitionbox}
...
% ── slides p.48–p.50 ──────────────────────────────────────────
\begin{definitionbox}{Liveness implies place-liveness}
\textbf{Proposition.} Every live net is place-live.
\par\smallskip
...
\]
The structure $\forall M \exists t$ is far weaker than liveness' $\forall t\, \forall M\, \exists M'$: one immediate step suffices, and the witness transition may depend on $M$. On the OG: a net is deadlock-free iff no node is a \emph{sink}: detecting deadlocks is one linear scan for nodes without outgoing arcs.
\end{definitionbox}
...
% ── slides p.58–p.60 ──────────────────────────────────────────
\begin{definitionbox}{Liveness implies deadlock-freedom}
\textbf{Lemma.} A live net is deadlock-free (a live, non-deadlock-free net does not exist).
\par\smallskip
...
\begin{itemize}
  ...
  \item \emph{Not live $\Rightarrow$ not place-live}: \textbf{false}. In the place-live-but-not-live witness above (or its variant where $t_5 : p_2 + p_3 \to p_1$ replaces $t_4$), the dead transition does not stop every place from being live: deadness of a transition does not propagate to its neighbour places.
...
% ── slides p.70–p.85 ──────────────────────────────────────────
\subsection{Exercises: the four-property checklist}
...
\end{tabular}
\end{center}
The patterns to remember: unbounded nets can be live and deadlock-free (A, C); a one-shot or structurally dead transition kills liveness without ever blocking the firing (B, D); only the cycle is bounded, and only bounded nets have finite OGs.
\end{examplebox}
