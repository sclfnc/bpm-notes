...
% ── slides p.4–p.8 ──────────────────────────────────────────
A first, informal annotation tags the communicating transitions: \texttt{?}-transitions (e.g.\ $\mathtt{?rec\_accept}$) fire when the corresponding message arrives, \texttt{!}-transitions (e.g.\ $\mathtt{!reject}$) produce a message visible to peers. Making the annotation structural turns the tags into \emph{interface places}: a set $P^I$ of \textbf{input places} feeding the \texttt{?}-transitions and a set $P^O$ of \textbf{output places} collecting tokens from the \texttt{!}-transitions. Interface places sit outside the workflow net but inside the module; interaction becomes ordinary token flow through them. The Seller module:
\begin{center}
...
\end{tikzpicture}
\end{center}
The receiving block $P^I = \{\mathtt{ra}, \mathtt{rr}\}$ feeds the \texttt{?}-transitions, the sending block $P^O = \{\mathtt{sa}, \mathtt{sr}\}$ collects from the \texttt{!}-transitions; everything inside the dashed boundary except the interface places is the original (sound) workflow net. Adding the interface places breaks the workflow-net shape (the net acquires sources besides $i$ and sinks besides $o$) and the enriched object gets its own name.
...
Compatibility becomes composition by \emph{fusing} the matching interface places: the sender's output place and the receiver's input place collapse into one shared place, and the modules become a single Petri net in which interaction is visible token flow. Closing the construction restores the workflow-net shape:
...
\end{definitionbox}
A workflow system is an ordinary workflow net (unique source, unique sink, every node on a path between them) so its soundness is checked with the machinery already in place (reachability graph, Commoner, free-choice results).
...
% FLAG 2026-07-10: a workflow-system figure for variant one was drawn by an agent but
% removed on verify: the AND-split wired transition->transition (missing the two branch
% places bI,bP), a bipartiteness violation, and the message-place fusion needs symbols
% (bI,bP) not named in the prose. Left to hand-drawing against the slides.
\end{examplebox}
...
A workflow net is \textbf{weakly sound} if it satisfies \emph{option to complete} (from every reachable marking the final marking is reachable) and \emph{proper completion} (when a token reaches $\mathbf{o}$, no token lingers elsewhere). Dead tasks are allowed. Weak soundness is decidable on the reachability graph and guarantees deadlock freedom and proper termination of all modules in the composition.
\end{definitionbox}
