...
% ── slides p.2–p.5 ──────────────────────────────────────────
The \emph{occurrence graph} formalises the behaviour of Petri nets; it then defines boundedness and, via \emph{coverability}, handles unbounded nets.\footnote{Desel J., Esparza J., \emph{Free Choice Petri Nets}, Cambridge University Press, \url{https://www7.in.tum.de/~esparza/bookfc.html}.}
...
\end{enumerate}
The algorithm terminates iff the reachable set is finite; the unbounded case is handled by the coverability construction at the end of the chapter.
...
\end{tikzpicture}
\end{center}
(\textsf{gg} = go-green, \textsf{gy} = go-yellow, \textsf{gr} = go-red; the cycle order is red $\to$ green $\to$ yellow $\to$ red.) The worklist run is short: exploring \textsf{red} adds the arc $\textsf{red} \xrightarrow{\textsf{gg}} \textsf{green}$; exploring \textsf{green} adds $\textsf{green} \xrightarrow{\textsf{gy}} \textsf{yellow}$; exploring \textsf{yellow} adds $\textsf{yellow} \xrightarrow{\textsf{gr}} \textsf{red}$, whose target is already a node. $\textit{Todo}$ empties and the OG is a 3-node directed cycle.
\end{examplebox}

% ── slides p.11–p.21 ──────────────────────────────────────────
\begin{examplebox}{Two independent traffic lights}
Two disjoint copies of the net (places \textsf{red}, \dots\ and \textsf{red'}, \dots), $M_0 = \textsf{red} + \textsf{red'}$. At every marking the enabled transitions of \emph{both} lights compete, so each exploration step enqueues up to two successors ($\textsf{red} + \textsf{red'}$ branches to $\textsf{green} + \textsf{red'}$ and $\textsf{red} + \textsf{green'}$, and so on). The construction stabilises on the full cartesian product: nine reachable markings
\[
...
Same single-light net, but $M_0 = 2\,\textsf{red}$: markings are multisets of two tokens over three places. From $2\,\textsf{red}$ only \textsf{go-green} is enabled (transitions fire one at a time; firing once removes a token, so the transition is not fired ``twice in parallel''), giving $\textsf{green} + \textsf{red}$; from there \textsf{go-green} again gives $2\,\textsf{green}$, while \textsf{go-yellow} gives \dots\ and the exploration closes after six nodes:
\begin{center}
...
\end{tikzpicture}
\end{center}
Six nodes against the nine of the two independent lights: same number of tokens, smaller state space, because the two tokens share the same places: the OG is the quotient of the product graph under the symmetry exchanging the tokens.
\end{examplebox}
...
German lights show \textsf{red+yellow} just before \textsf{green}: four phases, \textsf{red} $\to$ \textsf{red+yellow} $\to$ \textsf{green} $\to$ \textsf{yellow} $\to$ \textsf{red}. Tasks: model the four-state automaton, then a Petri net with three places for the lamps such that no impossible phase change is allowed. The WoPeD solution adds control places and a transition \textsf{go-ry} between \textsf{go-red} and \textsf{go-green}; with token counts listed as $(\textsf{red}, \textsf{yellow}, \textsf{green}, \textsf{control}_1, \textsf{control}_2)$ the reachability graph is the four-state cycle
\[
...
\begin{itemize}
  ...
  \item \emph{reachability graphs}: finite in the bounded variants (their size grows polynomially with producers, consumers, and capacity, and tool-generated drawings quickly become too dense to read); infinite when the buffer is uncapped.
\end{itemize}
\end{examplebox}

% ── slides p.46–p.50 ──────────────────────────────────────────
\begin{examplebox}{Dining philosophers}
Dijkstra posed the problem (five computers competing for five shared tape drives) as an exam question; Hoare popularised the dinner-table telling.\footnote{Dijkstra E.\,W., ``Hierarchical ordering of sequential processes'', \emph{Acta Informatica}, 1971; Hoare C.\,A.\,R., \emph{Communicating Sequential Processes}, 1985.} Five philosophers alternate between thinking and eating; the spaghetti requires two forks, one fork sits between each pair of neighbours, so \emph{no two neighbours may eat simultaneously}. Per philosopher $i$: places $\textsf{think}_i$ (marked) and $\textsf{eat}_i$; $\textsf{start\_eating}_i$ consumes $\textsf{think}_i$ and the two adjacent forks $f_i, f_{i+1 \bmod 5}$; $\textsf{start\_thinking}_i$ returns them:
\begin{center}
\begin{tikzpicture}
...
\end{tikzpicture}
\end{center}
The fragment is replicated five times around the table, each fork shared by two neighbours. Compute the reachability graph in WoPeD: it is finite, symmetric under the cyclic shift of the philosophers, and, because $\textsf{start\_eating}_i$ takes \emph{both} forks atomically, deadlock-free: an eating philosopher can always stop eating. The classical deadlock of the problem (every philosopher holding her left fork, waiting for the right one) appears only when fork-taking is split into two separate transitions; the textbook remedy is to have one philosopher pick up her forks in the opposite order.
\end{examplebox}
...
% ── slides p.62–p.65 ──────────────────────────────────────────
\begin{definitionbox}{Theorem: bounded $\iff$ finite OG}
