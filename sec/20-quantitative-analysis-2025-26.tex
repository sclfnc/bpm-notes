...
% ── slides p.2–p.3 ──────────────────────────────────────────
The chapters so far asked \emph{qualitative} questions about process models; this one turns models into numbers: throughput, waiting times, utilisation, cost. The exposition follows Ch.~7 of \emph{Fundamentals of Business Process Management}.\footnote{M.~Dumas, M.~La~Rosa, J.~Mendling, H.~A.~Reijers, \emph{Fundamentals of Business Process Management}, Ch.~7; course material at \url{https://courses.cs.ut.ee/2014/bpm/}.} Three concerns are easy to conflate. \textbf{Validation} relates the model to reality: does it capture the process the organisation actually carries out? \textbf{Verification} asks qualitative questions about the model itself (can it deadlock, can every case terminate, can two tasks run in either order?) the soundness-style questions of the previous chapters. \textbf{Performance analysis} asks quantitative ones: how many cases per hour, what average flow time, how many extra resources to reach a given throughput.
...
The loop inflates the $2$\,h check to $2.5$\,h; the rest composes as before.
\end{examplebox}
...
One eighth of the cycle time is actual work; the other seven eighths are waiting: a textbook candidate for waiting-time reduction.
\end{examplebox}
...
In 1954 the operations-research professor John Little postulated (without proof) that in a \emph{stable} system\footnote{Stable: the number of customers in the system does not grow unboundedly.} the long-term average number of customers equals the long-term average effective arrival rate times the average time a customer spends in the system:
$$WIP = \lambda \cdot CT.$$
\end{definitionbox}
The identity holds with no assumption on arrival or service distributions and no knowledge of the queueing discipline; Little proved it in 1961,\footnote{J.~D.~C.~Little, \emph{A proof of the queuing formula $L = \lambda W$}, Operations Research 9 (1961), 383--387; the classical statement reads $L = \lambda \cdot W$.} with simpler proofs following in 1967, 1969 and 1972. Two readings matter for redesign. First, $WIP$ grows with either $CT$ or $\lambda$: a slowing process accumulates cases, a faster inflow swells the active population; if $\lambda$ grows and $WIP$ must stay constant, $CT$ has to shrink. Second, since $WIP$ and $\lambda$ are easy to estimate by sampling, the law computes cycle time from two black-box observations, no flow analysis needed: $CT = WIP / \lambda$.
...
The loop sits on the costliest pair of activities and dominates the bill: the $500\,\euro$ body becomes $625\,\euro$ at $1.25$ visits per case.
\end{examplebox}
