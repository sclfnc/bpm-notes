...
% ── slides p.2–p.4 ──────────────────────────────────────────
\textbf{Petri nets} are a graphical and formal model of concurrent systems.\footnote{A deeper treatment of the tractable subclass of \emph{free-choice nets}: \url{https://www7.in.tum.de/~esparza/bookfc.html}} Petri nets fit business-process analysis because they are \emph{visual} (practitioners can read them), \emph{formal} (machines can analyse them), and \emph{tool-supported}. On that basis they support validation (testing behaviour on scenarios), verification (proving properties), and performance analysis (planning, dimensioning, optimisation). Automata model \emph{sequential} behaviour: one current state, one transition at a time. They serve protocol analysis, parsing, game-character logic, CPU control, speech recognition, vending machines, elevators, and traffic lights. What they cannot express directly is \emph{concurrency}: independent actions must be interleaved explicitly, exploding the state space. A Petri net is to a parallel system what a finite automaton is to a sequential one.
...
The same machinery scales to real controllers: a query-recognition automaton whose states are syntactic positions and whose transitions are labelled by words (accepting complete sentences such as \emph{``I want some information please''}); an ATM whose states are the screens of the dialogue (PIN check looping on failure, then deposit/withdrawal branches re-converging on card return); video-game characters (\textsc{standing}/\textsc{jumping}/\textsc{ducking}/\textsc{diving} driven by buttons, or an NPC cycling \textsc{wander}/\textsc{attack}/\textsc{evade} on player proximity and health). One state at a time, one transition fired, one symbol consumed.
...
% ── slides p.64–p.68 ──────────────────────────────────────────
\begin{examplebox}{Token game}
The smallest steps of the token game. Left: a transition with one input (two tokens) and one output (one token) fires, consuming one and producing one. Centre: a transition with \emph{two} inputs synchronises them: it fires only when both hold a token, consuming one from each. Right: if one input place is empty the transition is \emph{not enabled}, however many tokens the other input has. Bottom: a transition with two outputs forks, producing one token in each.
\begin{center}
\begin{tikzpicture}[node distance=0.7cm and 1.0cm]
  \node[bpmn event, minimum size=0.7cm] (p1) {};
  \node[bpmn task, rounded corners=0pt, minimum width=0.4cm, minimum height=0.7cm, right=of p1] (t1) {};
  \node[bpmn event, minimum size=0.7cm, right=of t1] (p2) {};
  \fill[accent] ([xshift=-2pt]p1.center) circle (2pt);
  \fill[accent] ([xshift=2pt]p1.center) circle (2pt);
  \fill[accent] (p2.center) circle (2pt);
  \draw[bpmn flow] (p1) -- (t1);
  \draw[bpmn flow] (t1) -- (p2);
  \node[bpmn event, minimum size=0.7cm, right=1.6cm of p2] (a1) {};
  \node[bpmn event, minimum size=0.7cm, below=of a1] (a2) {};
  \node[bpmn task, rounded corners=0pt, minimum width=0.4cm, minimum height=0.7cm] (at) at ($(a1)!0.5!(a2)+(1.3,0)$) {};
  \node[bpmn event, minimum size=0.7cm, right=0.9cm of at] (ao) {};
  \fill[accent] (a1.center) circle (2pt);
  \fill[accent] (a2.center) circle (2pt);
  \fill[accent] (ao.center) circle (2pt);
  \draw[bpmn flow] (a1) -- (at); \draw[bpmn flow] (a2) -- (at); \draw[bpmn flow] (at) -- (ao);
  \node[bpmn event, minimum size=0.7cm, right=1.6cm of ao, yshift=0.6cm] (b1) {};
  \node[bpmn event, minimum size=0.7cm, below=of b1] (b2) {};
  \node[bpmn task, rounded corners=0pt, minimum width=0.4cm, minimum height=0.7cm] (bt) at ($(b1)!0.5!(b2)+(1.3,0)$) {};
  \node[bpmn event, minimum size=0.7cm, right=0.9cm of bt] (bo) {};
  \fill[accent] ([xshift=-2pt]b1.center) circle (2pt);
  \fill[accent] ([xshift=2pt]b1.center) circle (2pt);
  \draw[bpmn flow] (b1) -- (bt); \draw[bpmn flow] (b2) -- (bt); \draw[bpmn flow] (bt) -- (bo);
  \node[font=\scriptsize, below=2pt of bt] {not enabled};
  \node[bpmn event, minimum size=0.7cm, below=2.2cm of p1] (c1) {};
  \node[bpmn task, rounded corners=0pt, minimum width=0.4cm, minimum height=0.7cm, right=0.9cm of c1] (ct) {};
  \node[bpmn event, minimum size=0.7cm] (co1) at ($(ct)+(1.3,0.5)$) {};
  \node[bpmn event, minimum size=0.7cm] (co2) at ($(ct)+(1.3,-0.5)$) {};
  \fill[accent] ([xshift=-2pt]c1.center) circle (2pt);
  \fill[accent] ([xshift=2pt]c1.center) circle (2pt);
  \fill[accent] (co2.center) circle (2pt);
  \draw[bpmn flow] (c1) -- (ct); \draw[bpmn flow] (ct) -- (co1); \draw[bpmn flow] (ct) -- (co2);
\end{tikzpicture}
\end{center}
\end{examplebox}

% ── slide p.70 ──────────────────────────────────────────
\begin{definitionbox}{Firing rule}
A transition is \emph{enabled} at a marking when each of its input places contains at least one token. Firing an enabled transition \textbf{consumes} one token from each input place and \textbf{produces} one token in each output place, atomically. Several transitions may be enabled at once; which fires is a non-deterministic choice: the freedom of an external scheduler.
\end{definitionbox}
...
Sets and multisets both ignore order; they differ on repetition: in a set each element appears at most once, in a \textbf{multiset} the number of occurrences is part of the data: the right type for markings, where a place may hold many tokens.
...
A \textbf{Petri net} is a tuple $(P, T, F, M_0)$ where $P$ is a finite set of \emph{places}, $T$ a finite set of \emph{transitions} with $P \cap T = \emptyset$, $F \subseteq (P \times T) \cup (T \times P)$ the \emph{flow relation} (bipartite arcs), and $M_0 : P \to \mathbb{N}$ (equivalently $M_0 \in \mu(P)$) the \emph{initial marking}. In general a \textbf{marking} $M : P \to \mathbb{N}$ gives the number of tokens in each place: the state of the net at an instant.
\end{definitionbox}
...
% ── slides p.90–p.95 ──────────────────────────────────────────
\begin{definitionbox}{Pre-set, post-set, enabling, firing}
For a node $x$ (place or transition), the \textbf{pre-set} and \textbf{post-set} are
\[
  {}^\bullet x \triangleq \{\, y \mid (y, x) \in F \,\}, \qquad x^\bullet \triangleq \{\, y \mid (x, y) \in F \,\}.
\]
A transition $t$ is \textbf{enabled} at $M$ iff ${}^\bullet t \subseteq M$ (the pre-set read as a multiset with unit multiplicities), written $M \xrightarrow{t}$ or $M[t\rangle$. An enabled transition can \textbf{fire}, producing
\[
...
On the order-fulfilment net: ${}^\bullet p_1 = \emptyset$, $p_1^\bullet = \{t_1\}$; ${}^\bullet t_2 = \{p_2\}$, $t_2^\bullet = \{p_3, p_4\}$; ${}^\bullet t_5 = \{p_5, p_6\}$, $t_5^\bullet = \{p_7\}$, and so on mechanically from $F$. Firing is atomic; only one transition fires at a time (\emph{interleaving} semantics); and the total number of tokens may grow or shrink with $|t^\bullet| - |{}^\bullet t|$: non-conservation lets nets model creation and destruction of resources without bookkeeping.

\begin{notebox}{WoPeD}
WoPeD\footnote{\url{http://woped.dhbw-karlsruhe.de/woped/}} (Workflow Petri Net Designer) is the free reference tool of the course: editing, token-game simulation (click enabled transitions to fire them), reachability and soundness analysis, standard import/export formats.
\end{notebox}
...
On the cycle net the loop $t_1 t_3 t_4 t_2$ returns to $p_1$ in four steps, so the infinite sequence $p_1 \xrightarrow{t_1 t_3 t_4 t_2\, t_1 t_3 t_4 t_2\, \cdots}$ is well-defined: the net is not terminating. Every prefix of an enabled infinite sequence ($\epsilon$, $t_1$, $t_1 t_3$, \dots) is itself a valid finite firing sequence, and the converse characterisation holds:

\begin{definitionbox}{Proposition: enabledness via prefixes}
...
\end{tikzpicture}}
\end{center}
The next chapter studies this graph as the \emph{occurrence graph} (reachability graph).
\end{examplebox}
