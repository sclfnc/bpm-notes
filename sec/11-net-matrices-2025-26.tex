...
% ── slides p.2–p.5 ──────────────────────────────────────────
\subsection{The unroll-once pattern}

% Edit 2026-06-12: dropped the course-admin sentence about the EPC/BPMN editor
% survey (OS support, pricing, mailto): logistics, not study content. The
% unroll-once exercise stays.
An exercise from the BPMN chapters finds its closure here. The exam collaboration (student/teacher) is modified so that \emph{at least one} exercise is assigned before the rate is computed: pull the first iteration out of the loop (a mandatory \textsc{Get exercise}/\textsc{Solve exercise} round on the student side, matched by \textsc{Assign exercise}/\textsc{Get solution} on the teacher side) and only then let an XOR gateway decide between repeating and exiting to \textsc{Get rate}/\textsc{Assign rate}. This \emph{unroll-once} pattern is the standard encoding of at-least-one loops.

% ── slides p.7–p.11 ──────────────────────────────────────────
\begin{examplebox}{Dining philosophers, revisited in WoPeD}
The net of the occurrence-graph chapter, per philosopher, places $\textsf{think}_i$/$\textsf{eat}_i$ and transitions $\textsf{start\_eating}_i$ (consuming both adjacent forks) and $\textsf{start\_thinking}_i$ (returning them), assembles into the full ring by sharing each fork place between two neighbours: $5 \cdot 2 + 5 = 15$ places and $5 \cdot 2 = 10$ transitions. WoPeD computes its reachability graph: finite (the net is safe), strongly connected, heavily symmetric (the five rotations of any marking are reachable), and \emph{without deadlocks}: the atomic two-fork grab makes mutual exclusion of neighbours structural, and an eating philosopher can always put the forks back. Nodes are labelled by bit-vectors recording who is eating; at most two non-adjacent philosophers eat simultaneously.
\end{examplebox}

% ── slides p.12–p.20 ──────────────────────────────────────────
\subsection{Vectors and matrices}

Linear algebra encodes Petri nets and their computations: the net becomes an integer matrix, markings become vectors, firing becomes vector addition, and a firing sequence is summarised by an integer vector counting its transitions. The structural-analysis machinery of the following chapters (invariants, soundness conditions) is built on this encoding.\footnote{Desel J., Esparza J., \emph{Free Choice Petri Nets}, Cambridge University Press, 1995, \url{https://www7.in.tum.de/~esparza/bookfc.html}.}

\begin{definitionbox}{Vectors, products, linear systems}
For a finite set $E = \{e_1, \ldots, e_n\}$, any map $v : E \to \mathbb{Q}$ (or $\mathbb{N}$, $\mathbb{Z}$) is a vector indexed by $E$; we use the same symbol for the row and the column form, the orientation being clear from the multiplication performed. The \emph{scalar product} of two equal-length vectors is $\mathbf{x} \cdot \mathbf{y} = \sum_i x_i y_i$ (row $\times$ column); a matrix-by-vector product computes one scalar product per row, a vector-by-matrix product one per column. A \emph{homogeneous linear system} is a matrix equation $A \cdot \mathbf{x} = \mathbf{0}$; its non-zero solutions are the invariants studied from the next chapter on, with place-invariants appearing as left null-vectors and transition-invariants as right null-vectors of the same matrix.
\end{definitionbox}
...
\end{array}
\]
Reading the matrix: the $+1/-1$ pair on rows $p_3, p_4$ in column $t_3$ records the coin moving from \emph{ready} to \emph{holding}; columns $t_4$ and $t_5$ both have $-1$ on $p_4$: accept and reject compete for the same holding token, a structural XOR. Every column sums to zero: each transition moves tokens without creating or destroying them, a property the next chapter names \emph{conservativeness}.
\end{examplebox}
...
\]
and computing the full product $\mathbf{N} \cdot \mathbf{y} = [0, 0, 0, 1, -1]^T$: the first three rows cancel, the last two are opposite. Vectors $\mathbf{y}$ for which such products vanish are the seeds of the invariants of the next chapter.

% ── slides p.61–p.67 ──────────────────────────────────────────
\subsection{Parikh vectors}
...
\begin{definitionbox}{Marking equation lemma}
...
On the vending machine, $M_0 + \mathbf{N} \cdot [0,1,2,1,1]^T = [4,0,1,0,0]^T + [-1,+1,0,0,0]^T = [3,1,1,0,0]^T$: the marking $3p_1 + p_2 + p_3$ reached by the step-by-step simulation, recovered without simulating. Two consequences: the reached marking depends \emph{only on the number} of occurrences of each transition, \emph{not on their order}. The qualifier is essential, the lemma is a one-way implication: it computes the effect of a sequence but says nothing about whether the sequence was enabled.

% ── slides p.77–p.82 ──────────────────────────────────────────
\subsection{Monotonicity}

% Added: the prefix characterisation of infinite enabledness was used in the
% monotonicity proof (below) and the repeatable-prefix corollary without being
% stated. Made it an explicit proposition so the infinite cases stand alone.
An infinite firing sequence has no last transition, so ``$M \xrightarrow{\sigma} M'$'' has no target marking to speak of; enabledness must be reduced to the finite case.
\begin{definitionbox}{Enabledness of infinite sequences (prefix characterisation)}
For $\sigma \in T^\omega$, write $M \xrightarrow{\sigma}$ (``$\sigma$ is enabled at $M$'') when the net can fire $\sigma$ forever from $M$. Then
\[
M \xrightarrow{\sigma} \quad\Longleftrightarrow\quad M \xrightarrow{\sigma'} \text{ for every finite prefix } \sigma' \text{ of } \sigma.
\]
\textit{Proof.} ($\Rightarrow$) Firing $\sigma$ fires each prefix. ($\Leftarrow$) Every transition of $\sigma$ occurs in some finite prefix, so if all prefixes are enabled the whole sequence fires step by step. \hfill$\square$
\end{definitionbox}

\begin{definitionbox}{Monotonicity lemma}
...
\begin{definitionbox}{Corollary (repeatable prefix) and two lemmas}
\textbf{Corollary.} If $M \xrightarrow{\sigma} M'$ with $M \le M'$, then $M \xrightarrow{\sigma\sigma\sigma\cdots}$.
...
\par\smallskip
\textbf{Boundedness lemma.} In a bounded system, $M \in [M_0\rangle$ with $M \ge M_0$ forces $M = M_0$.
...
\par\medskip
\textbf{Repetition lemma.} If $M \xrightarrow{\sigma} M'$ and $\sigma$ can be fired arbitrarily many times from $M$, then $M \le M'$.
...
\begin{examplebox}{Vending machine: diagnosing sequences by matrix algebra}
With $M_0 = 4p_1 + p_3$:
\par\smallskip
