...
% ── slides p.2–p.6 ──────────────────────────────────────────
\emph{Soundness} is the property that characterises a well-formed business process; the Petri-net machinery developed so far studies it.\footnote{Weske M., \emph{Business Process Management}, Springer, 2007, Ch.~6.} Boundedness and liveness, as defined earlier, are the right global properties for an \emph{ongoing} system; a workflow instead has a definite start and end per case, and soundness adapts them to that setting. Since BPMN, EPC and workflow nets share the same building blocks (start/end markers, splits/joins, tasks, links), the analysis is developed on workflow nets and transfers to the other notations through the usual mappings; the reseller process of the orchestration chapter (drawn in all three notations) is the running motivation.

% ── slides p.7–p.16 ──────────────────────────────────────────
\subsection{Workflow nets, structurally}

\begin{definitionbox}{Workflow net}
A Petri net $(P, T, F)$ is a \emph{workflow net} (WF net) if:
\begin{enumerate}
  \item there is a distinguished \emph{initial place} $i$ with ${}^\bullet i = \emptyset$;
  \item there is a distinguished \emph{final place} $o$ with $o^\bullet = \emptyset$;
  \item every other node lies on a path from $i$ to $o$.
\end{enumerate}
Places hold tokens representing \emph{cases}; transitions are \emph{activities}; a doubled border marks an activity refined by another workflow net. The canonical initial marking is one token on $i$.
\end{definitionbox}

The definition is purely structural, yet it already rejects several malformed shapes: a net with \emph{no} source or sink place (when would the case start or end?), a net with \emph{several} sources or sinks (which one starts or completes the case?), and \emph{dangling tasks} without inputs (when should they run?) or outputs (they contribute nothing to completion): condition 3 excludes them all. What it does not catch are vacuous decorations (a ``split'' with one output, a ``join'' with one input): syntactically legal, semantically misleading, left to later analysis. Terminology: a \emph{net system} is a net with an initial marking, the setting of behavioural properties; a \emph{net} alone is the setting of structural ones. For WF nets the two nearly coincide thanks to the canonical marking.
...
A \emph{livelock} traps a case in a cycle with no opportunity to end: from some reachable marking, every firing sequence keeps moving tokens around a sub-region without ever marking $o$. The system is not deadlocked (work happens) but the case progresses in circles. Livelocks are a failure class distinct from dead transitions and residual tokens.
\end{definitionbox}

All these flaws are detectable \emph{without knowing the goal} of the business process: they are intrinsic to the net's shape and dynamics, hence amenable to automatic model-level checking. This is \textbf{verification}: qualitative questions about the model (can it deadlock? do all cases terminate? can this task ever run?). It must be kept distinct from \textbf{validation}: the relation between model and reality (how does the model fit the event logs?), which is the business of process mining. The mismatched splits and joins behind these defects could often be prevented at design time; the structural conditions that guarantee prevention (free-choiceness, S/T-decompositions) come in the following chapters.
...
% ── slides p.54–p.67 ──────────────────────────────────────────
\subsection{Simulation, reachability, and their limits}

\emph{Simulation}: playing the token game, recording runs, randomising choices (WoPeD supports all three), finds defects but cannot prove their absence: how to be sure all runs were examined? \emph{Reachability analysis} is the exhaustive answer: all runs live in the reachability graph (coverability graph if unbounded), end states are evident, dangerous states can be searched. Its limit is \emph{state explosion}: the graph can be exponentially larger than the net, and a moderate workflow already produces an unreadable mesh. Worked diagnoses on a claim-handling net show what the analysis finds: an AND-join whose second input is never fed (deadlock); a place fed by both parallel branches and consumed once (tokens left after completion); checks still enabled after the case closed (pending activities); and a model covering only the success paths of its checks: failure transitions missing altogether, a business-completeness defect rather than a soundness one.
...
% ── slides p.74–p.81 ──────────────────────────────────────────
Checking the definition naively is expensive: condition 1 is one reachability question \emph{per transition}; condition 2 quantifies over \emph{all reachable markings}; condition 3 is a \emph{non}-reachability claim over the infinitely many markings $M > o$. The brute-force procedure works on the reachability graph: first check the net is a WF net (structural); then build the RG from $i$ and verify that every transition labels some arc (condition 1), that the RG has exactly one sink, equal to $o$, reachable from every node (conditions 2--3), and that no other node marks $o$. Automatic, but limited by state explosion, unhelpful for \emph{repairing} an unsound model, and inapplicable verbatim when the RG is infinite (the coverability graph is finite but loses exactness). Hence the structural strategy: translate soundness into liveness and boundedness, which have well-studied algorithms.
...
\begin{definitionbox}{Strong connectedness theorem}
...
% ── slides p.130–p.138 ──────────────────────────────────────────
\subsection{Worked example: the Computer Repair Service}
